...
const ShaderBackground = ({ className }: { className?: string }) => {
  ...
  const fsSource = `
    ...
    const float overallSpeed = 0.08; // Even slower
    ...
    const vec4 lineColor = vec4(0.08, 0.72, 0.65, 0.12); // Extremely subtle Teal
    const float minLineWidth = 0.005;
    const float maxLineWidth = 0.04; 
    const float lineSpeed = 0.5 * overallSpeed;
    const float lineAmplitude = 0.3; // Very flat
    const float lineFrequency = 0.12;
    const float warpSpeed = 0.1 * overallSpeed;
    const float warpFrequency = 0.2;
    ...
    void main() {
      ...
      vec4 lines = vec4(0.0);
      vec4 bgColor1 = vec4(1.0, 1.0, 1.0, 1.0); // Pure White Base
      vec4 bgColor2 = vec4(0.98, 1.0, 0.99, 1.0); // Soft Off-white
